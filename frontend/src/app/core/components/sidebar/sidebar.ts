...
<!-- Toggle Button -->
    ...
    <button (click)="toggleSidebar()"
        ...
        <!-- Icona -->
        <mat-icon class="!w-5 !h-5 !text-[20px] !text-gray-600 flex items-center justify-center leading-none">
    ...
    @if(!isCollapsed()){
      <div class="hover-scrollbar h-full overflow-y-auto">
          <!-- Header Categorie -->
          ...
          <div class="py-2 pb-10">
              ...
              <mat-accordion multi="true" displayMode="flat">
                  ...
                  <mat-expansion-panel class="!rounded-none !shadow-none border-b border-gray-50 bg-transparent" [expanded]="true">
  ...
  styles: [`
    ...
    .hover-scrollbar::-webkit-scrollbar-thumb { background-color: rgba(0,0,0,0.1); border-radius: 4px; }
  `]
})
export class CategorySidebarComponent implements OnInit {
  ...
  private groupCategoriesByParent(allCategories: Category[]): CategoryGroup[] {
    ...
    const rootCategories = allCategories.filter(c => c.parentProductCategoryId === null || c.parentProductCategoryId === undefined);
    
    // 2. Trova le categorie "figlie" (tutte le altre)
    const childCategories = allCategories.filter(c => c.parentProductCategoryId !== null && c.parentProductCategoryId !== undefined);
    ...
    const groups: CategoryGroup[] = rootCategories.map(parent => {
        return {
            ...
            categories: childCategories.filter(c => c.parentProductCategoryId === parent.productCategoryId)
        };
    });
